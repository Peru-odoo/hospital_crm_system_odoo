...
class DoctorSchedule(models.Model):
    ...
    time_slots = fields.One2many(
        "hospital.schedule.line", "schedule_id", string="Time Slots"
    )
...
class DoctorAvailabilitySlot(models.Model):
    ...
    schedule_id = fields.Many2one(
        "hospital.schedule", string="Availability", ondelete="cascade"
    )
    start_time = fields.Float(string="Start Time", required=True)
    end_time = fields.Float(string="End Time", required=True)
    is_slot_occupied = fields.Boolean(string="Occupied Slot", default=False)
    doctor = fields.Many2one(
        "hospital.doctor", string="Doctor", related="schedule_id.doctor_id"
    )
            ...
            same_day_slots = self.search(
                [
                    (
                        ...
                        "=",
                        slot.schedule_id.appointment_date,
                    ),
                    ("id", "!=", slot.id),
                ]
            )
