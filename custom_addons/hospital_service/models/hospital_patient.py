...
class Patient(models.Model):
    ...
    personal_doctor = fields.Many2one(
        "hospital.doctor", string="Personal Doctor", ondelete="restrict"
    )
    history_ids = fields.One2many(
        "hospital.personal_doctor_history", "patient", string="Doctor History"
    )
    ...
    def change_personal_doctor(self):
        return {
            ...
            "view_id": self.env.ref(
                "hospital_service.view_change_doctor_wizard_form"
            ).id,
            "target": "new",
            "context": {
                "default_new_doctor": self.personal_doctor
                and self.personal_doctor[0].id
                or False
            },
        }
